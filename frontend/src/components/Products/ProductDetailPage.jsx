 ...
 <section >
  {productdisplay
    ...
    .map((items) => (
       ...
       <img className="rounded-2xl object-cover h-[70px] w-[70px] filter invert" src={items.image_url} alt={items.name}/>
        ...
        <section className="flex flex-col gap-4 p-4 bg-white rounded-3xl shadow-2xl backdrop-blur-xl">
          ...
          {items.discount ? (
            ...
            <div className="text-2xl mt-2 font-semibold text-gray-800">Rs {items.price}</div>
          )}
      ...
      <h2 className='text-3xl text-gray-800 py-5 pl-7 font-semibold'>More from <p className='text-[#009b7e] relative flex'>{items.category}<img src='/leaf-nesture.png' className='h-5 w-10 relative -left-7 bottom-2'/></p></h2>
      <section className='relative flex items-center flex-wrap gap-6 m-5 '>
            {productdisplay.filter((ele)=>items.category==ele.category && ele.id !=items.id).map((item)=>{return <Link key={item.id} to={`/category/${item.category}/${item.id}`}> <div key={item.id} className="bg-[#E3E6EA] shadow-lg group rounded-3xl overflow-hidden w-full flex flex-col items-center">
