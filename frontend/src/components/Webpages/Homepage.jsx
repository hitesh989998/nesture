    ...
    `}
      ...
      <div className="flex justify-center p-3 ">
        <Link to="/all-products">
          <button className="bg-[#009b7e] text-white tracking-wide px-[22px] py-3 rounded-3xl mx-1 relative hover:bg-[#00765e] hover:text-white shadow-sm">
            All Products
          </button>
        </Link>
        <NavbarButtons
          className={
            'bg-[#F6F6F6] tracking-wide px-[11px] py-3 rounded-3xl mx-1 relative hover:bg-[#00765e] hover:text-white shadow-lg'
          }
        />
      </div>
        ...
        {AllProducts.map((item) => {
          return (
            <Link key={item.id} to={`/category/${item.category}/${item.id}`}>
              ...
              <div
                key={item.id}
                className="bg-[#E3E6EA] shadow-xl group rounded-3xl overflow-hidden w-full flex flex-col items-center"
              >
          ...
          <div className="">
            ...
            <div className="absolute bottom-0 right-0 p-3 m-3  h-[8rem] w-[49rem] backdrop-blur-md shadow-lg rounded-3xl tracking-wide bg-white bg-opacity-90">
              <p className="text-base leading-relaxed">
                ...
                From sustainable clothing to eco-friendly essentials, we've got
                you covered—all at the click of a button. Whether you have a
                question, need advice, or just want to share your thoughts, we’d
