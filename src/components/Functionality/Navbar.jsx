...
const Navbar = () => {
  ...
  return (
    <>      <div className={`${hamburger} bg-purple-600 w-screen h-screen overflow-hidden`}></div> 
    <nav className='shadow-md h-20 w-full flex items-center px-6'>
      <div className="flex gap-4">
      <button className='flex bg-[#F6F6F6] hover:bg-[#00765e] hover:text-white px-5 h-10 w-30 items-center gap-1 rounded-xl' onClick={handleVisiblity}><HiOutlineMenuAlt2 />Menu</button>
     ...
     <Link to='/'>
      ...
      <TbShoppingBag className='relative -mr-4 hover:bg-[#00765e] hover:text-white text-2xl bg-[#8FDAC5] h-10 w-10 p-2 rounded-full' />
       ...
       {isHoverCart && <div className='absolute right-32 top-16 h-auto w-30 bg-white bg-opacity-10 backdrop-blur-lg overflow-y-auto shadow-lg rounded-2xl p-5 m-2'>
       <CartPage/>
        </div>}
      </div>
      ...
      <div className='relative m-2 hover:bg-[#00765e] hover:text-white rounded-3xl  px-5 pl-10 h-15 w-30' onMouseOver={()=>changeHoverState(true)} onMouseLeave={()=>changeHoverState(false)}>
      <TbUserCircle className='absolute text-2xl h-10 w-10 text-white p-1 bg-[#E3E6EA] rounded-full right-32 mr-[1px] top-0.5' />
      <div className='py-1'><h3 className='text-sm font-medium pl-1'>Welcome, Guest!</h3>
      <h4 className='text-xs pl-1'>Access your account</h4></div>

      {isHover && <div className='absolute flex flex-col items-center left-0 bg-opacity-10 backdrop-blur-lg  bg-white h-38 w-38 shadow-sm rounded-2xl p-5 m-1.5 text-black'>
        <RiUserSmileLine className='text-4xl text-[#00765e]' />
        <h3 className='text-lg font-semibold'>User Account</h3>
        <h4 className='text-[#E3E6EA] text-sm text-center'>Access your Nesture account</h4>
        <button className='bg-[#00765e] hover:bg-[#6ABBA5] hover:text-white rounded-2xl p-2 mt-3 text-white flex text-base items-center'>Login<CiLogin className='text-2xl' /></button>
        </div>}
