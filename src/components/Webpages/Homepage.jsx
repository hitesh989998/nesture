...
const Homepage = () => {
  ...
  {AllProducts.map((item)=>{return <Link to={`/category/${item.category}/${item.id}`}> <div key={item.id} className="bg-white shadow-md rounded-lg overflow-hidden w-72 flex flex-col items-center">
    <img className='h-72 w-72'src={item.image_url}></img>
    <h2>{item.name}</h2>
    <p>{item.description}</p>
    {item.discount?<div><div className='line-through	'>{item.price}</div>
